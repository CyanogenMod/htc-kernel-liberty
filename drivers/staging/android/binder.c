					   ...
					   void __user *cookie)
{
	...
	if (node == NULL) {
		printk(KERN_ERR "binder: alloc_new_node_failed(%s:%d)\n", __func__, __LINE__);
		return NULL;
	}
						  ...
						  struct binder_node *node)
{
	...
	if (new_ref == NULL) {
		printk(KERN_ERR "binder: alloc_ref_failed(%s:%d)\n", __func__, __LINE__);
		return NULL;
	}
			       ...
			       struct binder_transaction_data *tr, int reply)
{
	...
	if (t == NULL) {
		printk(KERN_ERR "binder: alloc_transaction_failed(%s:%d)\n", __func__, __LINE__);
		return_error = BR_FAILED_REPLY;
	...
	if (tcomplete == NULL) {
		printk(KERN_ERR "binder: alloc_tcomplete_failed(%s:%d)\n", __func__, __LINE__);
		return_error = BR_FAILED_REPLY;
	...
	for (; offp < off_end; offp++) {
		...
		case BINDER_TYPE_FD: {
			...
			if (target_fd < 0) {
				printk(KERN_ERR "binder: get_unused_fd_failed(%s:%d)\n", __func__, __LINE__);
				fput(file);
				return_error = BR_FAILED_REPLY;
